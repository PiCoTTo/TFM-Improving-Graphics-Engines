...
namespace nimo
{
	...
	bool ExportedVariablesManager::addVariable(std::string name, glm::vec3& variable)
	{
		if (m_variables.find(name) == m_variables.end())
			m_variables[name] = std::make_shared<ExportedVariable>(name, typeid(glm::vec3), &variable);
