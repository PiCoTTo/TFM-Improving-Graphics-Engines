...
void RendererDebugPanel::OnRender(float deltaTime)
{
    ...
    frameTimes.push_back(m_gameViewPanel->getFrameTime());
    ...
    if (m_timeDebugRefresh > 1.f / m_refreshRate)
    {
        ...
        m_displayedStats.renderFrameTime = renderer->m_renderFrameTimer.ElapsedMillis();
        m_displayedStats.geometryFrameTime = renderer->m_geometryFrameTimer.ElapsedMillis();
        m_displayedStats.lightingFrameTime = renderer->m_lightingFrameTimer.ElapsedMillis();
        m_displayedStats.bloomFrameTime = renderer->m_bloomFrameTimer.ElapsedMillis();
        m_displayedStats.geometry2DFrameTime = renderer->m_geometry2DFrameTimer.ElapsedMillis();
